...
public class HotelClientStressTester implements Runnable {
 ...
 /*   void getRandomDateRange (Date d1, Date d2) {
    ...
    Record getRandomRecord () {
        ...
        SimpleDate today = MiscUtil.getToday();
        
        SimpleDate inDate = MiscUtil.getDateByDelta (today, i);
        SimpleDate outDate = MiscUtil.getDateByDelta (today, j);
        ...
        return MiscUtil.createRecord(0, guestID, hotel, type, inDate, outDate, 0);
    }
    ...
    void randomReserve () {
        ...
        ErrorAndLogMsg m = client.reserveHotel(
                ...
                r.getRoomType(), 
                r.getCheckInDate(), 
                r.getCheckOutDate(), id);
        
        r.setResID (id[0]);
        
        if (m==null || !m.anyError()) {
            ArrayList <Record> list = records.get(r.getGuestID());
            
            if (list ==null) {
                list = new ArrayList <Record> ();
                records.put(r.getGuestID(), list);
            }
    ...
    void randomCancelValid() {
        ...
        ErrorAndLogMsg m = client.cancelHotel(
                r.getGuestID(), r.getShortName(), r.getRoomType (), 
    ...
    void randomCancelInvalid () {
        ...
        if (list!=null) {
            for (Record rr : list) {
                int c1 = MiscUtil.compareDate (
                		rr.getCheckInDate(),
                		r.getCheckInDate() );
                
                int c2 = MiscUtil.compareDate (
                		rr.getCheckOutDate(),
                		r.getCheckOutDate() );
...
/*    void randomRecordQueryPerID () {
    ...
    void randomTransferRecordValid() {
        ...
        } while (toHotel.equals(r.getShortName() ));
        
        int [] idHolder = new int[1];
        idHolder[0] = r.getResID();
        ...
        if (m==null || !m.anyError()) {
            ...
            r.setResID ( idHolder[0] );
            r.setShortName (toHotel);
        }
        else if (m.error!=ErrorCode.ROOM_UNAVAILABLE)
            // there is a problem
            m.printMsg();
    ...
    public HotelClientStressTester(String args[]) {
        ...
        for (int i=0; i < serverNames.length; i++)
            serverNames[i] = servers[i].prof.getShortName();
    ...
    public void randomServiceReport() {
        ...
        SimpleDate date =  MiscUtil.getDateByDelta (today, i);
